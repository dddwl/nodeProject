...
Router.get('/async',async (req,res)=>{
    ...
    let data
    try{
        data = await operationDBS.find('order',{id:req.query.id*1});
    }catch(err){
        data = err;
    }
...
Router.put('/async',urlencoded,async(req,res)=>{console.log(req.body)
    let data
    try{
        data = await operationDBS.insert('order',{...req.body,add_time:Date.now()});
    }catch(err){
        data = err;
    }
...
Router.delete('/async',urlencoded,async(req,res)=>{
    ...
    try{
        data = await operationDBS.delete('order',{id});
    }catch(err){
        data = err;
    }
...
Router.post('/async',urlencoded,async(req,res)=>{
    ...
    let data
    try{
        data = await operationDBS.update('order',{id:req.body.id*1},{state:req.body.state});
    }catch(err){
        data = err;
    }
...
Router.post('/async1',urlencoded,async(req,res)=>{
    ...
    let data
    try{
        data = await operationDBS.update('order',{id:req.body.id*1},obj);
    }catch(err){
        data = err;
    }
